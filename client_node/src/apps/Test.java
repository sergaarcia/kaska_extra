...
class Test {
    ...
    static private boolean doCommit(KaskaClient cl, Scanner ent) throws RemoteException {
        System.err.println("Introduzca el nombre del tema y offset a salvar (en blanco si quiere salvar todos los suscritos con el offset actual): ");
        if (!ent.hasNextLine()) return false;
        String lin = ent.nextLine();
        Scanner s = new Scanner(lin);
	String topic = null;
	int offset = 0;
        ...
        boolean res;
	if (topic==null) res = cl.commit(); else res = cl.commit(topic, offset);
        System.err.println("commit ha devuelto un valor " + res);
        return true;
    }
    static private boolean doCommitted(KaskaClient cl, Scanner ent) throws RemoteException {
        System.err.println("Introduzca el nombre del tema cuyo offset se quiere recuperar (en blanco si quiere recuperar el offset de todos los suscritos): ");
        if (!ent.hasNextLine()) return false;
        String lin = ent.nextLine();
        Scanner s = new Scanner(lin);
	String topic = null;
        if (s.hasNext()) topic = s.next();
        boolean res;
	if (topic==null) res = cl.committed(); else res = cl.committed(topic);
        System.err.println("committed ha devuelto un valor " + res);
        return true;
    }
    static private void prompt() {
        System.err.println("Introduzca operacion (Ctrl-D para terminar)");
        System.err.println("\toperaciones: createTopics|topicList|sendString|SendClass|get|subscribe|unsubscribe|position|seek|poll|commit|committed");
    }
    ...
    static public void main(String args[]) {
        if (args.length!=3) {
            System.err.println("Usage: Test registryHost RegistryPort Client");
            return;
        }
        try {
            KaskaClient cl;
            cl = new KaskaClient(args[0], args[1], args[2]);
            if (cl == null) return;
            while (true) {
                ...
                if (s.hasNext()) {
                   ...
                   switch (op) {
                       ...
                       case "commit": formatoOK = doCommit(cl, ent); break;
                       case "committed": formatoOK = doCommitted(cl, ent); break;
                    }
                }
                if (!formatoOK)
